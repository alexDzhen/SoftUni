...
public class WrongMeasurements {
    public static void main(String[] args) {
        ...
        int[] positions = Arrays.stream(scanner.nextLine().split("\\s+")).mapToInt(Integer::parseInt).toArray();
        ...
        for (int row = 0; row < matrix.length; row++) {
            for (int col = 0; col < matrix[row].length; col++) {
                if (hasHistory(wrongPositionsLog, row, col)) {
                    int newValue = 0;
                    if (isInWithinTheMatrix(n, (row - 1)) && !hasHistory(wrongPositionsLog, row - 1, col)) {
                        newValue = newValue + matrix[row - 1][col];
                    }
                    if (isInWithinTheMatrix(n, (row + 1)) && !hasHistory(wrongPositionsLog, row + 1, col)) {
                        newValue = newValue + matrix[row + 1][col];
                    }
                    if (isInWithinTheMatrix(matrix[row].length, (col - 1)) && !hasHistory(wrongPositionsLog, row, col - 1)) {
    ...
    private static boolean hasHistory(List<String> wrongPositionsLog, int row, int col) {
        return wrongPositionsLog.contains((row) + " " + (col));
    }
}
